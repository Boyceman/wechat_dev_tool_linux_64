'use strict';var _exports;function init(){function a(I,J){let K=JSON.parse(decodeURIComponent(J)),L=K.file,M=K.msg.codeFrame;I.executeScript({code:`console.group("${new Date} js 文件，编译错误")
      ...
      console.groupEnd()`})}function o(I,J){let K=J.details,L=K.error||`the server responded with a status of ${K.statusCode} (${K.statusLine})`,M=`Failed to load ${K.type} ${K.url} : ${L} `,N=K.ip?`From server ${K.ip}`:'';I.executeScript({code:`console.group("${new Date} 渲染层网络层错误")
        console.error(\`${M}\n${N}\`)
      console.groupEnd()`})}const p=require('../../../config/config.js'),q=require('../../../weapp/utils/parseErr.js'),r=require('url');_exports={};const{WEBVIEW_NETWORK_ERROR:s,WXML_LOSE_ERROR:t,WXSS_IMPORT_ERROR:u,WXSS_ERROR:v,WEBVIEW_ERROR:w,JSON_ENTRANCE_ERROR:x,PAGE_DEFINE_ERROR:y,PAGEJS_FILE_ERROR:z,COMPONENT_FOR_DEVELOPER:A,ES6_ERROR:B,WXML_ERROR:C,JSON_FILE_ERROR:D,JSON_PARSE_ERROR:E,WXML_RUNTIME_ERROR:F,JSON_CONTENT_ERROR:G}=p,H={'-1':'debug',0:'info',1:'warn',2:'error'};_exports=(I,J,K)=>{J===A?b(I,K):J===C?c(I,K):J===B?a(I,K):J===D?d(I,K):J===E?f(I,K):J===z?h(I,K):J===y?i(I,K):J===F?j(I,K):J===x?e(I,K):J===w?k(I,K):J===v?l(I,K):J===u?m(I,K):J===t?n(I,K):J===G?g(I,K):J===s&&o(I,K)}}init(),module.exports=_exports;